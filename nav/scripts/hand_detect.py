...
# Initialize ROS node
rospy.init_node('hand_detection_node_pyzed') # Changed node name to distinguish
...
# Bridge to convert ROS Image messages to OpenCV images (still useful if you want to publish processed images back to ROS)
bridge = CvBridge()

# Initialize ZED camera
zed = sl.Camera()
init_params = sl.InitParameters()
init_params.camera_resolution = sl.RESOLUTION.HD720  # or sl.RESOLUTION.HD1080 or sl.RESOLUTION.VGA
init_params.camera_fps = 30 # Set camera framerate
...
# Create a sl.Mat object to store the image
image_zed = sl.Mat()
...
def main():
    rate = rospy.Rate(30)  # Set a rate for the main loop, e.g., 30 Hz
    ...
    while not rospy.is_shutdown():
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            cv_image = image_zed.get_data() # Get the image as a numpy array
            cv_image = cv2.flip(cv_image,0)

            # Convert BGRA (ZED's default) to BGR for OpenCV or RGB for MediaPipe
            ...
            if pose_results.pose_landmarks:
                ...
                if is_landmark_visible(right_elbow) and is_landmark_visible(right_shoulder) and is_landmark_visible(right_hip) and is_landmark_visible(left_elbow) and is_landmark_visible(left_wrist):
                ...
                if is_landmark_visible(left_elbow) and is_landmark_visible(left_shoulder) and is_landmark_visible(left_hip) and is_landmark_visible(right_elbow) and is_landmark_visible(right_wrist):
...
def cleanup():
    ...
    zed.close() # Close the ZED camera when the node shuts down
